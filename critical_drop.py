...
def drop_points( points, target,modeladv,criterion,numdrop,numstep):
    ...
    for i in range(numstep):
        ...
        for j in range(points_adv.shape[0]):
            squeeze_idx = []
            count_idx = np.zeros((points_adv.shape[2],), dtype=np.int32)
            for idx in range(drop_indice.shape[1]):
                count_idx[drop_indice[j][idx]] += 1
            for idx in range(points_adv.shape[2]):
                if count_idx[idx] != 0:
                    squeeze_idx.append(idx)

            # drop_indice_j = np.random.choice(np.array(squeeze_idx), self.a, replace=False)
            drop_indice_j = np.argpartition(count_idx, kth=points_adv.shape[2] - numdrop)[-numdrop:]
...
if __name__ == '__main__':
    ...
    for j, data in enumerate(test_loader, 0):
        ...
        cur_batch_data_adv = drop_points(points, label, modeladv, loss, num_drop, num_steps)
