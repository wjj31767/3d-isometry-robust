...
class Generator(nn.Module):
    def __init__(self, num_points=2048,num_classes=40,bradius=1.0,up_ratio = 4,in_channel=379,in_channel1=168):
        ...
        self.fp3 = PointNetFeaturePropagation(3,[64])
        self.fp2 = PointNetFeaturePropagation(3,[64])
        self.fp1 = PointNetFeaturePropagation(3,[64])
        ...
        for out_channel in [64, 3]:
            ...
            self.mlp_bns1.append(nn.BatchNorm1d(out_channel))
            last_channel = out_channel
    def forward(self, xyz, labels_onehot):
        B, num_point, _ = xyz.shape
        ...
        labels_onehot = torch.tensor(labels_onehot, torch.float32)
        labels_onehot = torch.unsqueeze(labels_onehot, 1)
        labels_onehot = torch.unsqueeze(labels_onehot, 1)
        labels_onehot = labels_onehot.repeat(1, num_point, 1, 1)
        net = torch.cat([net, labels_onehot], 3)
